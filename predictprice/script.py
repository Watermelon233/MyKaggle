...
def munge(df):
    ...
    all_df["MasVnrArea"].fillna(df["MasVnrArea"].mean(), inplace=True)
   
    all_df["BsmtFinSF1"] = df["BsmtFinSF1"]
    all_df["BsmtFinSF1"].fillna(df["BsmtFinSF1"].mean(), inplace=True)

    all_df["BsmtFinSF2"] = df["BsmtFinSF2"]
    all_df["BsmtFinSF2"].fillna(df["BsmtFinSF2"].mean(), inplace=True)

    all_df["BsmtUnfSF"] = df["BsmtUnfSF"]
    all_df["BsmtUnfSF"].fillna(df["BsmtUnfSF"].mean(), inplace=True)

    all_df["TotalBsmtSF"] = df["TotalBsmtSF"]
    all_df["TotalBsmtSF"].fillna(df["TotalBsmtSF"].mean(), inplace=True)
    ...
    all_df["GarageArea"].fillna(df["GarageArea"].mean(), inplace=True)
    ...
    all_df["BsmtFullBath"].fillna(df["BsmtFullBath"].mean(), inplace=True)

    all_df["BsmtHalfBath"] = df["BsmtHalfBath"]
    all_df["BsmtHalfBath"].fillna(df["BsmtHalfBath"].mean(), inplace=True)
    ...
    all_df["GarageCars"].fillna(df["GarageCars"].mean(), inplace=True)
    ...
    all_df["PoolArea"].fillna(df["PoolArea"].mean(), inplace=True)
    ...
    all_df["Neighborhood_Good"].fillna(all_df["Neighborhood_Good"].mean(), inplace=True)
...
ldf = label_df.as_matrix()

x,xt,y,yt = train_test_split(ldf, label_df["SalePrice"], train_size = 0.75)
